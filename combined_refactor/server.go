...
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	handlers := map[string]func(json.RawMessage){
		...
		"subscription_get": func(data json.RawMessage) {
			...
			if err := json.Unmarshal(data, &params); err != nil {
				session.sendWSMessage("subscription_error", "subscription_get 参数解析失败")
				return
			}
		...
		"subscription_save": func(data json.RawMessage) {
			...
			if err := json.Unmarshal(data, &params); err != nil {
				session.sendWSMessage("subscription_error", "subscription_save 参数解析失败")
				return
			}
			...
			if items, listErr := listSubscriptions(); listErr == nil {
				session.sendWSMessage("subscription_list", items)
			}
		},
		"subscription_update": func(data json.RawMessage) {
			...
			if err := json.Unmarshal(data, &params); err != nil {
				session.sendWSMessage("subscription_error", "subscription_update 参数解析失败")
				return
			}
			id := strings.TrimSpace(params.ID)
			if id == "" {
			...
			safeGo("subscription-update", session, func() {
				session.sendWSMessage("subscription_update_started", map[string]interface{}{"id": id})
				ctx, cancel := context.WithTimeout(r.Context(), 30*time.Second)
				defer cancel()
				item, err := updateSubscription(ctx, id)
				if err != nil {
					session.sendWSMessage("subscription_update_error", map[string]interface{}{"id": id, "message": err.Error(), "item": item})
					if items, e := listSubscriptions(); e == nil {
						session.sendWSMessage("subscription_list", items)
					}
					return
				}
		...
		"subscription_delete": func(data json.RawMessage) {
			var params subscriptionIDRequest
			if err := json.Unmarshal(data, &params); err != nil {
				session.sendWSMessage("subscription_error", "subscription_delete 参数解析失败")
				return
			}
			id := strings.TrimSpace(params.ID)
			if id == "" {
			...
			session.sendWSMessage("subscription_deleted", map[string]string{"id": id})
			if items, e := listSubscriptions(); e == nil {
				session.sendWSMessage("subscription_list", items)
			}
		},
